...
function formatDate(dateObject) {
  //permet d'afficher la date complète avec l'heure
...
fetch(`${BACKEND_URL}/bookings`)
  ...
  .then((data) => {
    ...
    for (let i = 0; i < data.result.length; i++) {
      ...
      const timeBefore =
        (Date.parse(data.result[i].date) - new Date()) / (3, 6e6);
      ...
      document.querySelector("#booking-elements").innerHTML += `
          <div class="booking-trip">
            ...
            <div class="dep-date">
                <p class="booking-departure">${String(days).padStart(
                  2,
                  "0"
                )}-${String(months).padStart(2, "0")}-${years} at ${String(
        hours
      ).padStart(2, "0")}:${String(minutes).padStart(2, "0")}</p>
            </div>
            <div class="before">
                <p id="time-before">Departure in ${Math.round(
                  timeBefore
                )} hours</p>
            </div>
